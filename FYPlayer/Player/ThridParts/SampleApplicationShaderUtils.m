...
+ (GLuint)createProgramWithShader:(NSString *)shader
    ...
    if(shader && shader.length){
        ...
        // 3
        const char * shaderStringUTF8 = [shader UTF8String];
        GLint shaderStringLength = (GLint)[shader length];
        ...
        if (compileSuccess == GL_FALSE) {
            ...
            NSLog(@"Error compiling shader (%@): %@", @"VERTEX effect base", messageString);
            return 0;
        }
        
        GLuint fragmentShader = [self compileShader:fragmentShaderFileName withDefs:nil withType:GL_VERTEX_SHADER];
        
        if ((shaderHandle == 0) || (fragmentShader == 0)) {
        ...
        if (linkSuccess == GL_FALSE) {
            ...
            NSString *messageString = [NSString stringWithUTF8String:messages];
            NSLog(@"Error linkink shaders (%@): %@", @"VERTEX effect base", messageString);
            return 0;
        }
